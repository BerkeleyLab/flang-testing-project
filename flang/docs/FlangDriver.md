...
script will:
* use `flang` to unparse the input source file (i.e. it will run `flang
...
`flang` as follows:
```bash
cmake -DCMAKE_Fortran_FLAGS="-flang-experimental-exec" -DCMAKE_Fortran_COMPILER=<path/to/flang> <src/dir>
```
...
use the `-flang-experimental-exec` flag when invoking `flang` as in the
example above.
