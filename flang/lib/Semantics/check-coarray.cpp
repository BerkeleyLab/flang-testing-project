    ...
    if (const auto *expr{GetExpr(context, statOrErrmsg)}) {
      if (ExtractCoarrayRef(expr)) {
        context.Say(parser::FindSourceLocation(statOrErrmsg),
            "A stat-variable or errmsg-variable in a %s may not be a coindexed object"_err_en_US,
            listName);
      }
    }
  };
...
static void CheckEventVariable(
  ...
  if (const auto *expr{GetExpr(context, eventVar)}) {
    if (!IsEventType(evaluate::GetDerivedTypeSpec(expr->GetType()))) {
...
void CoarrayChecker::Leave(const parser::EventWaitStmt &x) {
  ...
  // Additional check on event-variable that only applies to EventWaitStmt
